...
ADMIN_SERVICE_FILE="${ADMIN_SERVICE_FILE:-/etc/systemd/system/ai-robot-edge-admin.service}"
...
pip install -e "${APP_DIR}[audio,servo,vision]"
...
install -m 0644 "${APP_DIR}/deploy/ai-robot-edge-admin.service" "${ADMIN_SERVICE_FILE}"
systemctl daemon-reload

echo "Edit ${CONFIG_DIR}/edge.yaml, then run:"
echo "  systemctl enable --now ai-robot-edge"
echo "  systemctl enable --now ai-robot-edge-admin"
