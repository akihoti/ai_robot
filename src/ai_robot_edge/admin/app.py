...
PARAMETER_FIELDS: tuple[ParameterField, ...] = (
    ...
    ParameterField("voice.local_welcome_enabled", "启用本地欢迎", "语音", "bool", "人脸稳定后播放本地欢迎音频，不触发远端欢迎事件"),
    ParameterField("voice.welcome_audio_path", "本地欢迎音频", "语音", "text", "本地欢迎 WAV 文件路径"),
    ParameterField("voice.presence_listening_enabled", "人脸在场持续监听", "语音", "bool", "人脸在场时使用持续监听模式"),
    ParameterField("voice.face_absence_stop_listening_seconds", "离场停听延时", "语音", "float", "人脸消失后停止监听的确认延时（秒）", min_value=0, max_value=30, step=0.1),
    ...
    ParameterField("vad.engine", "VAD 引擎", "VAD", "select", "语音活动检测后端", ("energy", "silero_onnx")),
    ParameterField("vad.model_path", "Silero 模型路径", "VAD", "text", "Silero ONNX 模型文件路径"),
    ParameterField("vad.threshold", "Silero 起音阈值", "VAD", "float", "高于该概率判定为语音", min_value=0.01, max_value=0.99, step=0.01),
    ParameterField("vad.negative_threshold", "Silero 判停阈值", "VAD", "float", "低于该概率累计静音", min_value=0.01, max_value=0.98, step=0.01),
    ParameterField("vad.min_speech_ms", "最短语音", "VAD", "int", "低于该时长的片段视作噪声", min_value=30, max_value=3000, step=10),
    ParameterField("vad.cooldown_ms", "VAD 冷却", "VAD", "int", "短噪声后暂停触发的时间", min_value=0, max_value=5000, step=50),
    ...
    ParameterField("speaker.normalize_loudness", "统一播放音量", "设备", "bool", "播放前按目标响度统一 TTS/欢迎语音量"),
    ParameterField("speaker.target_rms_dbfs", "目标响度", "设备", "float", "播放归一化目标 RMS dBFS", min_value=-36, max_value=-6, step=0.5),
    ParameterField("speaker.peak_limit", "峰值限制", "设备", "float", "归一化后的最大峰值比例", min_value=0.1, max_value=1.0, step=0.01),
    ParameterField("speaker.max_output_channels", "最大输出声道", "设备", "int", "限制异常声卡报告的输出声道数", min_value=1, max_value=8, step=1),
